...
EXPOSE 8080
RUN wget -q -O '/tmp/voyant.zip' 'https://github.com/sgsinclair/VoyantServer/releases/download/2.4.0-M45/VoyantServer2_4-M45.zip'
RUN mkdir /tmp/voyant/ && cd /tmp/voyant/ && unzip -qq /tmp/voyant.zip &&\
    rm -rf /usr/local/tomcat/webapps/ROOT &&\
    rm -rf /usr/local/tomcat/webapps/voyant &&\
    cp -r /tmp/voyant/VoyantServer2_4-M45/_app /usr/local/tomcat/webapps/voyant &&\
